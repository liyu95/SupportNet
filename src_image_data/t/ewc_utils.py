...
import time
np.random.seed(1997)
def build_graph_ewc(hyper_params,fixed_params):
    ...
    tf_networks={}
    #change to 'test'
    tf_networks['train_network']=ResNet('ResNet32','train',
                                        num_outputs=fixed_params['total_num_classes'],name='ResNet32_train',use_fisher=True)
    ...
    tf_tensors['class_loss_train']=tf.reduce_sum(
               ...
               labels=tf_tensors['Y_one_hot_train'],
                logits=tf_networks['train_network'].tf_tensors['fc'])*tf_tensors['sample_weight_train'])/tf.reduce_sum(tf_tensors['sample_weight_train'])
    
    #tf_tensors['regularization_loss_train']=hyper_params['beta']*tf_networks['train_network'].tf_tensors['l2_loss']
    
    tf_tensors['ewc_loss_train']=hyper_params['ewc_lambda']*tf_networks['train_network'].tf_tensors['ewc_loss']
    tf_tensors['loss_train']=tf_tensors['class_loss_train']+tf_tensors['ewc_loss_train']
...
#     tf_tensors['optimizer_train']=tf.train.AdamOptimizer(learning_rate=tf_variables['lr']).minimize(tf_tensors['loss_train'])
    ...
    tf_networks['test_network']=ResNet('ResNet32','test',num_outputs=fixed_params['total_num_classes'],name='ResNet32_test',use_fisher=True)
    ...
    tf_tensors['ewc_loss_test']=hyper_params['lambda']*tf_networks['test_network'].tf_tensors['ewc_loss']
    tf_tensors['loss_test']=tf_tensors['class_loss_test']+tf_tensors['ewc_loss_test']
    ...
    tf_networks['prev_network']=ResNet('ResNet32','test',num_outputs=fixed_params['total_num_classes'],
                                       name='ResNet32_previous',use_fisher=True)
...
def fit_ewc(tf_tensors,tf_variables,tf_networks,fixed_params,hyper_params,data_dict_total,session,resume=False,
    ...
    if tf_log_var_val['global_step']==0 and tf_log_var_val['iteration_step']==0 and pretrain_evaluation>=2:
        ...
        evaluation_tensors=dict(loss=tf_tensors['loss_test'],
                                ...
                                regularization_loss=tf_tensors['ewc_loss_test'],
        ...
        if verbose>=2:
            print_var_list=['loss_train','class_loss_train',
                           'loss_test','class_loss_test',
    ...
    for iteration in range(tf_log_var_val['iteration_step'],num_iterations):
        ...
        if iteration>=1:
            #prev dataset
            using_classes=class_ord[(iteration-1)*fixed_params['class_batch_size']:iteration*fixed_params['class_batch_size']]
            train_idx=np.array([i in using_classes for i in data_dict_total['Y_train']])
            test_idx=np.array([i in using_classes for i in data_dict_total['Y_test']])
            ...
            fisher_info_val=previous_network.get_fisher_info(data_dict_prev['X_test'],session)
            rv_val=previous_network.get_all_rv(session)
            tf_networks['train_network'].load_model(previous_best_model_params_file,session)
            tf_networks['train_network'].set_fisher_var(fisher_info_val,session)
            tf_networks['train_network'].set_prev_rv(rv_val,session)
...
#                 print(session.run(list(tf_networks['train_network'].prev_variables.values())))
#                 print({v:session.run(tf_networks['train_network'].prev_variables[v]) for v in ['conv_1/W','resconv1_4a/W','fc/W']})
#                 print({v:session.run(tf_networks['train_network'].fisher_variables[v]) for v in ['conv_1/W','resconv1_4a/W','fc/W']})
#                 print(session.run(list(tf_networks['train_network'].fisher_variables.values())))
                tf_networks['test_network'].set_prev_rv(tf_networks['train_network'].prev_variables,session)
                tf_networks['test_network'].set_fisher_var(tf_networks['train_network'].fisher_variables,session)                 
...
#                 tf_log_var_val['top5_accuracy_train']+=local_top5_accuracy/num_batches_train
            ...
            train_tensors['regularization_loss_train']=tf_tensors['ewc_loss_train']
...
#                                                      session)
            ...
            model_params=tf_networks['train_network'].get_all_model_params(session)
            for k in tf_networks['train_network'].regularizable_variables:
                if np.max(model_params[k])==np.inf or np.any(np.isnan(model_params[k])):
                    print(k)
                    assert False
            tf_networks['test_network'].set_model_params(model_params,session)
            tf_networks['test_network'].set_prev_rv(tf_networks['train_network'].prev_variables,session)
    ...
    #                 print(k+' '+str(tf_log_var_val[k])+' '+str(type(tf_log_var_val[k])))
        ...
        tf_networks['test_network'].reset_ewc_variables(session)
