...
def check_feature_representation(f_size, f_lam, s_size):
    ...
    print('load model: fsize_{}_flam_{}_ssize_{}_class_{}.ckpt'.format(f_size, int(f_lam), s_size, 2))
    saver_load.restore(sess,
        '../model/fsize_{}_flam_{}_ssize_{}_class_{}.ckpt'.format(f_size, int(f_lam), s_size, 2))
    data_1 = exclude_data(data_all, range(2,6))
    predict_label_train,predict_label,final_train_fea,final_test_fea=whole_set_check(data_1)
    evaluate_model(data_1[5],np.array(predict_label))
    evaluate_model(data_1[0],np.array(predict_label_train))
    ...
    for s_class in range(3,7):
        print('load model: fsize_{}_flam_{}_ssize_{}_class_{}.ckpt'.format(f_size,int(f_lam), s_size, s_class))
        saver_load.restore(sess,
            '../model/fsize_{}_flam_{}_ssize_{}_class_{}.ckpt'.format(f_size,int(f_lam), s_size, s_class))
